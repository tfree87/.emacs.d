...
#+begin_src elisp :tangle yes :comments org :noweb-ref dired_custom
  (with-eval-after-load 'dired
    (require 'dired-x))
#+end_src

** Move/Rename/Copy Files to Another Window

This will cause Emacs to default to moving/copying/renaming files from the directory in one Dired buffer.
 #+begin_src emacs-lisp :noweb-ref dired_custom :comments org
   (dired-dwim-target t)
 #+end_src
