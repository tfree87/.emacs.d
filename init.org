...
#+date: Time-stamp: <2021-12-21 12:20:18 Thomas>
...
#+end_src
* Track Load Time
When enabled, benchmark-init will track the load times of the Emacs init file
  #+begin_src emacs-lisp :tangle yes
...
*** All the icons
~all-the-icons~ adds lots of useful icons to ~ivy-mode~, ~ibuffer~, and ~dired~ that quickly identifies each item in lists.
#+begin_src emacs-lisp :tangle yes
...
#+end_src
*** Emacs Theme
Change the default color and mode line themes in Emacs.
**** Spacemacs Theme
I like the colors used in Spacemacs so here is a way to get that color-scheme installed in vanilla Emacs.
#+begin_src emacs-lisp :tangle yes
 ...
 #+end_src
**** Use Spaceline them for Mode Line
Change the basic mode line to one that looks a bit more like the mode line in Spacemacs.
#+begin_src emacs-lisp :tangle yes
...
Adds icons to the mode line using ~all-the-icons~. This is currently disabled because I have not been able to make it look right. 
  #+begin_src emacs-lisp :tangle yes
...
#+end_src
* Coding
This section contains customizations for editiing programming code using Emacs.
** Numpydoc
I prefer python docstrings to be written in Numpy format and the Numpydoc package provides an interactive way to create these docstrings.
#+begin_src emacs-lisp :tangle yes
...
#+end_src
** Elpy
Elpy adds some improvements for editing python code in Emacs.
#+begin_src emacs-lisp :tangle yes
  (use-package elpy
    :ensure t
    :defer t
...
#+end_src
** Paren Mode
Paren Mode shows matching parenthesis making it easy to see which set of parentheses are closed.
#+begin_src emacs-lisp :tangle yes
...
#+end_src
* BBDB
Tbe Big Brother DataBase (BBDB) stores contact information in a text file and works well with Emacs mail readers and org-mode. 
#+begin_src emacs-lisp :tangle yes
  (use-package bbdb
    :ensure t
    :defer t
...
#+end_src
* Docker
Docker can be managed from within Emacs 
...
#+end_src
* Eshell
Eshell is an operating system independent command line shell that uses elisp to implemewnt many shell functions natively within Emacs. This makes it highly customizable and is highly useful if you spend a lot of time in Emacs as output can be redirected to Emacs buffers.
** Set up Eshell
...
#+end_src
* ibuffer
ibuffer greatly improves the buffer-list in Emacs.
#+begin_src emacs-lisp :tangle yes
  (use-package ibuffer
    :defer t
    :bind
...
#+end_src
* TRAMP
Set TRAMP to use a bash shell by default instead of the remote shell as zsh seems to cause issues in TRAMP.
#+begin_src elisp :tangle yes
  (eval-after-load 'tramp '(setenv "SHELL" "/bin/bash"))
#+end_src
* Performance Optimization End
This last block of code closed the performance optimization from the top of the file.
#+begin_src emacs-lisp :tangle yes
